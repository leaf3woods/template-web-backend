...
Reserve the `*Options` suffix for values bound from an executable host's configuration sources and consumed through the .NET options pattern. Use `*Settings` for equivalent runtime or per-call values that are created in code and passed directly. Configuration options declare their section name, are registered in the composition root, and validate required values at startup; runtime settings are not configuration-bound or injected through `IOptions<T>`. When settings selectively override configured defaults, merge them once in the owning adapter and validate the resolved result. Prefer nullable members when omission must be distinguished from valid default values; a documented sentinel is also acceptable when it cannot be a valid value. Continue to use semantic names such as `Request`, `Command`, and `Dto` when those meanings are more precise than `Settings`.
